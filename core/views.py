...
from .forms import DocFileForm, DocumentForm
from .models import Cart, Product_List, Seller
...
def HomeView(request):
    ...
    if list_all:
        product = [x for x in list_all.order_by("-rating") if x.total_ratings > 10000][
            :10
        ]
        trending = [
            x for x in list_all.order_by("-total_ratings") if x.total_ratings > 1631200
        ][:10]
        latest = [
            x
            for x in list_all.order_by("-total_ratings", "-rating")
            if x.total_ratings < 1500 and x.rating > 4
        ][:10]
        return render(
            request,
            "home.html",
                            ...
                            additional=json.dumps(
                                {
                                    ...
                                    "pages": i["pages"],
                                    "language": i["language"],
                                    "publication date": i["publication date"],
                                }
                            ),
...
def Seller_Landing(request, cat, store, pk):
    ...
    product = [
        x
        for x in Product_List.objects.filter(seller=pk).order_by("-rating")
        if x.total_ratings > 100
    ][:10]
    trending = [
        x
        for x in Product_List.objects.filter(seller=pk).order_by("total_ratings")
        if x.rating > 4 and x.total_ratings > 10
    ][:10]
    latest = [
        x for x in Product_List.objects.filter(seller=pk).order_by("total_ratings")
    ][:10]
    return render(
        request,
        "seller.html",
...
def Product_Dscr(request, pk):
    ...
    return render(request, "product_dscr.html", {"data": record, "seller": slr})
...
def Profile(request):
    try:
        ...
        return render(request, "profile.html", {"base_user": usr})
        # except:
        #     return redirect('404')
...
def Products(request):
    ...
    try:
        ...
        sort = (
            sort
            if sort in ["price_asc", "price_dsc", "rating", "trend", "fresh", "most"]
            else "rating"
        )
    except:
        sort = "rating"
    ...
    if sort == "price_asc":
        ...
        object_list = [
            x
            for x in object_list.order_by("total_ratings")
            if x.rating > 4 and x.total_ratings > 10
        ]
    elif sort == "fresh":
    ...
    if lang:
        object_list = [
            x for x in object_list if json.loads(x.additional)["language"] == lang
        ]

    # tmp = object_list.all()
    if not object_list:
        return render(request, "search.html", {"found": False, "search": product})
                ...
                if pk is not None:
                    try:
                        item = Cart.objects.get(
                            customer_id=request.user.id, product_id=pk, status="Cart"
                        )
                        item.nos += 1
                        item.save()
                    except:
...
def CartView(request):
    ...
    cart = Cart.objects.filter(customer_id=request.user.id, status="Cart").order_by(
        "-date"
    )
    tmp = []
    for x in cart:
